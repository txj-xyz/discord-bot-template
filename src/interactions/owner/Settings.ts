...
import { ApplicationCommandOptionChoiceData, AutocompleteInteraction, ChatInputCommandInteraction, EmbedBuilder, SlashCommandBooleanOption, SlashCommandStringOption, codeBlock } from 'discord.js';
import Bot from '../../Bot';
...
export default class Settings extends BotInteraction {
    constructor(client: Bot) {
        ...
        this.slashData
            ...
            .addStringOption((option: SlashCommandStringOption) => option.setName('option').setDescription('Which setting to toggle').setAutocomplete(true).setRequired(true))
            .addBooleanOption((option: SlashCommandBooleanOption) => option.setName('action').setDescription('Enable / Disable this setting?').setRequired(true));
    }
    ...
    async run(interaction: ChatInputCommandInteraction<'cached'>): Promise<void> {
        ...
        const [option, action] = [interaction.options.getString('option', true), interaction.options.getBoolean('action', true)];
